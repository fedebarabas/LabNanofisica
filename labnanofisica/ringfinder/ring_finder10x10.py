...
#        self.setGrid(self.outputVb,n=10)
    ...
    def FFT2(self, data):
        ...
        coord = peak_local_max(fft2output, min_distance=2, threshold_rel=self.fftThr)
...
#            thetaSteps = np.arange(0,(180/n),1)
            ...
            for i in thetaSteps:
                for p in phaseSteps:
                    axonTheta = simAxon(subImgSize, wvlen, deltaAngle[i], p*.025, a=0, b=sinPower).simAxon;
...
#            R = arrayExt(R)
    ...
    def getDirection(self, data):
        ...
        # get directions
        lines = probabilistic_hough_line(edges, threshold=10, line_length=dataSize*linLen,
                                         line_gap=3)
